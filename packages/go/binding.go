...
// BreadcrumbTarget is an enumeration type.
type BreadcrumbTarget string

const (
	// BreadcrumbTargetContent BreadcrumbTargetContent inert (#1393). Previously prepended the heading breadcrumb into chunk
	...
	BreadcrumbTargetContent BreadcrumbTarget = "content"
	// BreadcrumbTargetMetadata BreadcrumbTargetMetadata inert (#1393), and was already a no-op on `content` before this change.
	// Kept only for backward compatibility, since `Content` is no longer
	// distinguishable from it.
	BreadcrumbTargetMetadata BreadcrumbTarget = "metadata"
)
...
const (
	...
	// OutputFormatStructured metadata-only label; content is identical to [`OutputFormat::Plain`].
...
// Sealed interface -- use one of CredentialProviderConfigAzureAd, CredentialProviderConfigVertexOauth2.
...
// Sealed interface -- use one of HostMatcherExact, HostMatcherSuffix.
...
type ChunkingConfig struct {
	...
	//
	// Default: `Content`.
	BreadcrumbTarget BreadcrumbTarget `json:"breadcrumb_target,omitempty"`
}
...
func (s *ChunkingConfig) UnmarshalJSON(data []byte) error {
	...
	s.BreadcrumbTarget = raw.BreadcrumbTarget
...
// EnsureInitialized ensure built-in extractors are registered.
//
...
// InstallPdfRenderDiagnostics install [`PdfOxideWarningCapture`] as the process-wide `log` backend,
// exactly once.
//
...
func InstallPdfRenderDiagnostics() bool {
	ptr := C.xberg_install_pdf_render_diagnostics()
	return ptr != 0
}

// TakePdfOxideRenderWarnings drain the glyph-drop [`ProcessingWarning`]s accumulated on this thread by
...
func TakePdfOxideRenderWarnings() []ProcessingWarning {
	ptr := C.xberg_take_pdf_oxide_render_warnings()
	return func() []ProcessingWarning {
		if ptr == nil {
			return nil
		}
		defer C.xberg_free_string(ptr)
		var result []ProcessingWarning
		if err := json.Unmarshal([]byte(C.GoString(ptr)), &result); err != nil {
			return nil
		}
		return result
...
// BuildDecoderPromptTokens build the four (or three) token Whisper decoder prompt.
//
...
func BuildDecoderPromptTokens(startOfTranscript uint32, langID uint32, transcribe uint32, noTimestamps uint32, timestamps bool) []int64 {
	cStartOfTranscript := C.uint32_t(uint32(startOfTranscript))

	cLangID := C.uint32_t(uint32(langID))

	cTranscribe := C.uint32_t(uint32(transcribe))

	cNoTimestamps := C.uint32_t(uint32(noTimestamps))

	var cTimestamps C.int32_t
	if timestamps {
		cTimestamps = 1
	} else {
		cTimestamps = 0
	...
	ptr := C.xberg_build_decoder_prompt_tokens(cStartOfTranscript, cLangID, cTranscribe, cNoTimestamps, cTimestamps)
	return func() []int64 {
		if ptr == nil {
			return nil
		}
...
// TimestampTokenToMs convert a raw Whisper timestamp token ID to a millisecond offset from the
...
func TimestampTokenToMs(tokenID uint32, timestampBeginID uint32) uint32 {
	cTokenID := C.uint32_t(uint32(tokenID))

	cTimestampBeginID := C.uint32_t(uint32(timestampBeginID))

	ptr := C.xberg_timestamp_token_to_ms(cTokenID, cTimestampBeginID)
	return uint32(ptr)
}

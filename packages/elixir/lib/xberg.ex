...
defmodule Xberg do
  ...
  @doc "Ensure built-in extractors are registered."
  @spec ensure_initialized() :: {:ok, nil} | {:error, atom, String.t()}
  def ensure_initialized do
    Xberg.Native.ensure_initialized()
  end
  ...
  @spec doctor() :: map()
  def doctor do
    Xberg.Native.doctor(nil)
  end

  @doc "Probe the backends and settings in `config` and report what will actually execute on this host."
  @spec doctor(String.t() | nil) :: map()
  def doctor(config) do
    Xberg.Native.doctor((cond do is_nil(config) -> nil; is_binary(config) -> config; true -> Jason.encode!(config) end))
  end

  @doc "Install [`PdfOxideWarningCapture`] as the process-wide `log` backend, exactly once."
  @spec install_pdf_render_diagnostics() :: boolean()
  def install_pdf_render_diagnostics do
    Xberg.Native.install_pdf_render_diagnostics()
  end
  ...
  @spec build_decoder_prompt_tokens(
          non_neg_integer(),
          non_neg_integer(),
          non_neg_integer(),
          non_neg_integer(),
  ...
  def build_decoder_prompt_tokens(start_of_transcript, lang_id, transcribe, no_timestamps, timestamps) do
    Xberg.Native.build_decoder_prompt_tokens(start_of_transcript, lang_id, transcribe, no_timestamps, timestamps)
  end

  @doc "Convert a raw Whisper timestamp token ID to a millisecond offset from the start of the 30-second chunk it was decoded in."
  @spec timestamp_token_to_ms(non_neg_integer(), non_neg_integer()) :: non_neg_integer()
  def timestamp_token_to_ms(token_id, timestamp_begin_id) do
    Xberg.Native.timestamp_token_to_ms(token_id, timestamp_begin_id)
  end

    ...
    pub fn new(opts: std::collections::HashMap<String, rustler::Term>) -> Self {
        Self {
            ...
            table_chunking: opts
                ...
                .and_then(|t| t.decode().ok())
                .unwrap_or_default(),
            breadcrumb_target: opts
                .get("breadcrumb_target")
                .and_then(|t| t.decode().ok())
                .unwrap_or_default(),
        }
    }
...
#[allow(clippy::derivable_impls)]
impl Default for BreadcrumbTarget {
    fn default() -> Self {
        Self::Content
    }
}
...
pub fn ensure_initialized() -> Result<(), String> {
    let result = xberg::extractors::ensure_initialized().map_err(|e| e.to_string())?;
    Ok(result)
}
...
#[rustler::nif]
pub fn doctor(config: Option<String>) -> DoctorReport {
    let config_core: Option<xberg::ExtractionConfig> =
        config.and_then(|s| serde_json::from_str::<xberg::ExtractionConfig>(&s).ok());
    xberg::doctor(config_core.as_ref().unwrap_or(&Default::default())).into()
}
...
pub fn extractionconfig_validate(obj: ExtractionConfig) -> Result<(), String> {
    let result = xberg::ExtractionConfig::from(obj)
        .validate()
        .map_err(|e| e.to_string())?;
    Ok(result)
}
...
impl From<CredentialProviderConfig> for xberg::CredentialProviderConfig {
    fn from(val: CredentialProviderConfig) -> Self {
        match val {
            ...
            CredentialProviderConfig::VertexAdc { scope } => Self::VertexAdc { scope: scope },
...
impl From<NodeContent> for xberg::NodeContent {
    fn from(val: NodeContent) -> Self {
        match val {
            ...
            NodeContent::Comment { text } => Self::Comment { text: text },
            NodeContent::Group {
                label,
                heading_level,
                heading_text,

...
The Wasm package ships Tesseract as a separate module. Call `enableOcr()` after `initWasm()` to load it, then pass the `tesseract-wasm` backend in the config:

<Snip_wasm_ocr_enable_ocr />

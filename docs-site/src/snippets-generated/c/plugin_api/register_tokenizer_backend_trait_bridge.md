...
static size_t sample_count_tokens(const void *user_data, const char * text) {
    (void)user_data;
    (void)text;
    return 0;
}
...
int main(void) {
    ...
    XBERGXbergTokenizerBackendVTable vtable = {
        .name_fn = sample_name,
        .version_fn = sample_version,
        .initialize_fn = sample_initialize,
        .shutdown_fn = sample_shutdown,
        .count_tokens = sample_count_tokens,
        .free_string = sample_release_callback_string,
        .free_user_data = sample_free_context,
    };
    char *error = NULL;
    int32_t status = xberg_register_tokenizer_backend("test-backend", &vtable, context, &error);
    if (status != 0) {
        xberg_free_string(error);
        return EXIT_FAILURE;
    }
    status = xberg_unregister_tokenizer_backend("test-backend", &error);
    if (status != 0) {
        xberg_free_string(error);
        return EXIT_FAILURE;
    }

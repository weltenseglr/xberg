---
id: fixture_c_register_reranker_backend_trait_bridge
language: c
target: c
level: typecheck
requires: []
...
static int32_t sample_rerank(const void *user_data, const char * query, const char * documents, char ** out_result, char ** out_error) {
    (void)user_data;
    (void)query;
    (void)documents;
    *out_result = NULL;
...
int main(void) {
    ...
    XBERGXbergRerankerBackendVTable vtable = {
        .name_fn = sample_name,
        .version_fn = sample_version,
        .initialize_fn = sample_initialize,
        .shutdown_fn = sample_shutdown,
        .rerank = sample_rerank,
        .free_string = sample_release_callback_string,
        .free_user_data = sample_free_context,
    };
    char *error = NULL;
    int32_t status = xberg_register_reranker_backend("test-backend", &vtable, context, &error);
    if (status != 0) {
        xberg_free_string(error);
        return EXIT_FAILURE;
    }
    status = xberg_unregister_reranker_backend("test-backend", &error);
    if (status != 0) {
        xberg_free_string(error);
        return EXIT_FAILURE;
    }
